...
function sendToOptions(message){
            ...
            chrome.tabs.query({url: optionsUrl}, function(tabs) {
                if (tabs.length) {
                    ...
                    //console.log('option tabid = ' + tabs[0].id)
 ...
 * 
 * 
 * Initialiser le gestionnaire d'événements message en arrière-plan
 * 
 * 
 * 
 * 
...
chrome.runtime.onMessage.addListener((request,sender,response)=>{           
    //Table de branchement pour traitre selon le message reçu du popup
    switch(request.message){
        ...
        case 'horaire':
            window.tabId = false;
            //Vérifier l'onglet horaire enseignant et le sélectionner
            recherche_onglet('horaire_enseignant');    
            //Récupérer le numéro de id de l'onglet
            chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
                window.localStorage.setItem('tabId',tabs[0].id); //Enregistre dans storage   
                
                //Envoyer un message sur l'onglet pour extraire l'horaire
                chrome.tabs.sendMessage(tabs[0].id, {message: "extracthorraire"}, function(response) {
                    console.log(response.message); //Affiche le contenu de l'horaire  
                
                    var ical = generer_ics(response.message);
                   
                  //Transmettre l'horaire vers option  
                  sendToOptions(ical);  
                  });    
                  
                //Envoyer un message sur l'onglet pour générer le fichier ics
                chrome.tabs.sendMessage(tabs[0].id, {message: "generateics"}, function(response) {
                    console.log(response.message);
                  });                    
        ...
        case 'fermer':
            //terminer les processus et vider le storage local      
            break;
      
        default:
            console.log('message : vide/invalide');
    }    
